...
const FULLMOON_LAYOUT = [
  { x: 84,  y: 0   },  // 1: 지금 삶    (top center)
  { x: 0,   y: 102 },  // 2: 인간관계   (upper left)
  { x: 168, y: 102 },  // 3: 방해 요소  (upper right)
  { x: 0,   y: 306 },  // 4: 극복 방법  (lower left)
  { x: 168, y: 306 },  // 5: 행동 지침  (lower right)
  { x: 84,  y: 408 },  // 6: 외부 교훈  (bottom center)
  { x: 84,  y: 204 },  // 7: 예상 결과  (center — 보름달 에너지)
] as const;
...
export default function TarotFullMoonPage() {
  ...
  async function handleInterpret() {
    ...
    try {
      const res = await fetch("/api/fortune/tarot-full-moon", {
        method: "POST",
          ...
          >
            ...
            <div className="relative mx-auto" style={{ width: MOON_W, height: MOON_H }}>
              {drawn.map((d, i) => {
                ...
                const isCenter = i === 6; // 카드 7 (예상 결과) — 보름달 중심
                  ...
                  >
                    ...
                    <div
                      className={`absolute -top-2 -left-2 w-[16px] h-[16px] rounded-full text-white text-[9px] font-bold flex items-center justify-center ${
                        isCenter
                          ? "bg-sky-900 border border-sky-400/60"
                          : "bg-indigo-900 border border-indigo-500/60"
                      }`}
                      style={{ zIndex: 20 }}
                    >
                      {i + 1}
